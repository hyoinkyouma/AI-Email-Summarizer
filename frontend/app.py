...
local_ip = os.getenv('STREAMLIT_SERVER_ADDRESS') 
st.markdown("""
    <style>
        /* Remove top padding */
        .block-container {
...
if st.button("Fetch Digest"):
    with st.spinner("Generating summary..."):
        r = requests.get(f"http://{local_ip}:8009/daily-digest")
        if r.ok:
...
if data:
    ...
    if st.button("Speak Summary"):
        with st.spinner("Generating voice..."):
            r_audio = requests.post(f"http://{local_ip}:8009/synthesize", json={"text": st.session_state.summary_clean})
            if r_audio.ok:
